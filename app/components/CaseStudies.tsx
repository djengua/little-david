...
export default function CaseStudies() {
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-8 py-12 sm:py-24 border-b border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-4 sm:space-y-6">
        {caseStudies.map((caseStudy) => {
          ...
          return (
            <div
              key={caseStudy.id}
              className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl border-l-4 border-accent hover:shadow-lg transition-all"
            >
              <div className="inline-flex items-center gap-2 bg-gradient-to-r from-accent to-accent-dark text-white px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-bold mb-3 sm:mb-4 uppercase tracking-wider font-mono">
                <IconComponent className="w-3 sm:w-4 h-3 sm:h-4" />
                {caseStudy.label}
              </div>
              <h3 className="text-lg sm:text-2xl font-bold text-primary dark:text-white mb-2 sm:mb-3 font-mono">
                {caseStudy.title}
              </h3>
